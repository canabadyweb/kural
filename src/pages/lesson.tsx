...
const Lesson: NextPage = () => {
  ...
  const kuralLessons = (a) => { return  a.map ( a => kuralLessonProblem(a));}
    ...
    const isStartingLesson = true;
    const setIsStartingLesson = 1;
    ...
    const checkCorrectAnswer = (correctAnswer) => {
      if (Array.isArray(correctAnswer))
      {
        if(correctAnswer.length == 2){
          const answer1 = numbersEqual(selectedAnswers, correctAnswer[0]);
          const answer2 = numbersEqual(selectedAnswers, correctAnswer[1]);

          return answer1 ? answer1 : answer2;
        } else {
          return numbersEqual(selectedAnswers, correctAnswer);
        } 
      } else {
        return Number(selectedAnswer) == correctAnswer
      }
    }

    // const isAnswerCorrect = Array.isArray(correctAnswer)
    //   ? numbersEqual(selectedAnswers, correctAnswer)
    //   : Number(selectedAnswer) === correctAnswer;

    const isAnswerCorrect = checkCorrectAnswer(correctAnswer);
    const onCheckAnswer = () => {
    ...
    switch (problem.type) {
      ...
      case "WRITE_IN_THAMIZH": {
        return (
          <ProblemWriteInThamizh
            problem={problem}
            correctAnswerCount={correctAnswerCount}
